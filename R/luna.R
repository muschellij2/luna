...
lsl <- function(file , path = "") {
  ...
  if (dim(d)[2] < 2)
    stop("invalid sample list")
  cat(dim(d)[1], "observations in", file, "\n")
  if (dim(d)[1] > length(unique(d[, 1])))
    stop("duplicate IDs found")
  l <- list()
  for (i in 1:dim(d)[1]) {
    l[[d[i, 1]]]$EDF <-
      ifelse(path == "" , d[i, 2] , paste(path, d[i, 2], sep = "/"))
    a <- d[i, -c(1:2)]
    a <- a[a != ""]
    if (path != "")
...
lattach <- function(sl, idx = "")  {
  ...
  if (is.numeric(idx)) {
    if (idx < 1 |
        idx > length(sl))
      stop(paste("idx out of range, expecting 1 .." , length(sl)))
    id = names(sl)[idx]
  } else {
    idx <- which(names(sl) == id)
    if (length(idx) != 1)
      stop(paste("could not find index", idx))
  }
  ...
  if (!is.null(val)) {
    ...
    d <-
      read.table(var ,
                 ...
                 stringsAsFactors = FALSE)
    if (dim(d)[2] != 2)
      stop(paste("expecting two tab-delimited columns in" , v))
    n <- dim(d)[1]
    for (i in 1:n)
      .Call("Rset_var" , as.character(d[i, 1]) , as.character(d[i, 2]) , PACKAGE =
              "luna")
  }
  invisible(1)
}
...
lflush <- function()
{
  if (luna.globals$logmode)
    .Call("Rflush_log" , PACKAGE = "luna")
...
ladd.annot <- function(annot , intervals)
{
  if (!is.list(intervals))
    stop("expecting a list of intervals")
  # check that each have two of each, convert to vector
  t <- unlist(intervals)
  if (length(t) != 2 * length(intervals))
    stop("bad interval list format: expecting two items per list element")
  if (length(t) != 0)
    .Call("Radd_annot_fromR" ,
          as.character(annot) ,
          as.numeric(t) ,
          PACKAGE = "luna")
...
{
  mapply(c , (e - 1) * inc , (e - 1) * inc + dur , SIMPLIFY = FALSE)
}
...
lannots <- function(a = "") {
  ...
  if (length(a) != 1)
    stop("lannots( annot ) only takes one annotation class")
...
{
  if (missing(sl))
    stop("no sample list 'sl' specified")
  if (missing(x))
    stop("no Luna commands 'x' specified")
  ids <- names(sl)
  if (length(ids) == 0)
    stop("no individuals in sample-list")
...
lcmd <- function(filename) {
  ...
  for (i in 1:length(lines))
  {
    if (substr(lines[i] , 1, 1) == " ") {
      if (prv_line == "")
        stop("badly formed command line continuation")
      prv_line =	   paste(prv_line	, lines[i])
    } else {
      if (prv_line != "")
        newlines <- c(newlines , prv_line)
      prv_line = lines[i]
    }
  }
  if (prv_line	!= "")
...
lstgcols <- function(s) {
  as.vector(sapply(s , function(x) {
    ifelse(x == "NREM1" ,
           rgb(0, 190, 250, 255, maxColorValue = 255) ,
           ifelse(
             x == "NREM2" ,
             rgb(0, 80, 200, 255, maxColorValue = 255) ,
             ifelse(
               x == "NREM3" ,
               rgb(0, 0, 80, 255, maxColorValue = 255) ,
               ifelse(
                 x == "NREM4" ,
                 rgb(0, 0, 50, 255, maxColorValue = 255) ,
                 ifelse(
                   x == "REM"   ,
                   rgb(250, 20, 50, 255, maxColorValue = 255) ,
                   ifelse(
                     x == "wake"  ,
                     rgb(100, 100, 100, 255, maxColorValue = 255) ,
                     rgb(20, 160, 20, 100, maxColorValue =
                           255)
                   )
                 )
               )
...
lbands <- function(l) {
  invisible(capture.output({
    ...
    leval(paste(
      "FILTER sig=",
      l,
      "_delta bandpass=0.5,4 tw=1 ripple=0.02" ,
      sep = ""
    ))
    leval(paste(
      "FILTER sig=",
      l,
      "_theta bandpass=4,8   tw=1 ripple=0.02" ,
      sep = ""
    ))
    leval(paste(
      "FILTER sig=",
      l,
      "_alpha bandpass=8,12  tw=1 ripple=0.02" ,
      sep = ""
    ))
    leval(paste(
      "FILTER sig=",
      l,
      "_sigma bandpass=12,15 tw=1 ripple=0.02" ,
      sep = ""
    ))
    leval(paste(
      "FILTER sig=",
      l,
      "_beta  bandpass=15,30 tw=1 ripple=0.02" ,
      sep = ""
    ))
  }))
}
